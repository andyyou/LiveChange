...
class PollViewModel {
    ...
    var poll: Poll? = nil
    var activity: Activity<LiveChangeWidgetAttributes>?
    ...
    func startActivityIfNeeded() {
        guard let poll = self.poll, activity == nil, ActivityAuthorizationInfo().frequentPushesEnabled else {
            return
        }
        
        if let currentPollIdActivity = Activity<LiveChangeWidgetAttributes>.activities.first(
            where: { activity in
            ...
            do {
                let activityAttributes = LiveChangeWidgetAttributes(pollId: pollId)
